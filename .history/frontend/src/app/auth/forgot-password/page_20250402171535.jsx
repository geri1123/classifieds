    ...
    <div className="flex bg-white dark:bg-gray-800 rounded-lg py-5 px-7 flex-col gap-4 w-[550px] shadow-lg">
      ...
      {error && (
        <div className="flex gap-2 items-center">
          <CgDanger className="text-red-700" />
          <p className="text-red-700 font-base text-sm">{error}</p>
        </div>
      )}
        <div className={`flex items-center border rounded-lg border-gray-300 pl-3 py-1 ${error ? "border-red-500 ":""}`}>
          <AiOutlineMail className={`text-gray-500 w-5 h-5 ${error?"text-red-500":''}`} />
          <input
            type="email"

...
router.put("/api/update-product-status/:id", verifyToken, async (req, res) => {
  const userId = req.userId;
  ...
  try {
    // Update the product status based on ENUM values
    const updateQuery = `UPDATE products SET status_type = ? WHERE id = ?`;
    await promisePool.query(updateQuery, [status_type, productId]);

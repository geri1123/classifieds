...
router.get("/social-media" , verifyToken , (res,req)=>{
    const userId=req.userId;
    const socialMediaQuery="SELECT * FROM `social_media` WHERE user_id = ?";
...
router.put("/add-social-media", verifyToken, (req, res) => {
  ...
  const updateQuery = `
      ...
      fiverr = VALUES(fiverr);
  `;

  db.query(updateQuery, [userId, instagram, facebook, linkedin, fiverr], (err, result) => {
      if (err) {

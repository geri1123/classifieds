...
export default function ProductsByCategory() {
  ...
  const { status } = router.query; // Now you're extracting 'status' from the query parameter
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    if (!status) return; // Don't run the API call if there's no status
    ...
    fetchProducts();
  }, [status]); // Run the fetch function again if the status changes

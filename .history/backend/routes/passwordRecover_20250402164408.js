...
router.post("/forgot-password", (req, res) => {
  ...
  db.query(checkEmailQuery, [email], (err, results) => {
    ...
    res.json({ 
      message: "We have emailed the link to reset your password !", 
...
router.post("/reset-password", (req, res) => {
  ...
  if (!newPassword) {
    return res.status(400).json({ error: "Fill in the new password !" });
  }
  if (newPassword.length < 8) {
    return res.status(400).json({ error: "The password must contain at least 8 characters !" });
  }
  if(!repeatPassword){
    return res.status(400).json({ error: "Fill in the 'Repeat Password' field !" });
  }
  if(newPassword !==repeatPassword){
    return res.status(400).json({ error: "Recurring password does not match new password !" });
  }

...
router.post("/add-product", verifyToken, upload.array("product_img", 20), async (req, res) => {
    ...
    if (categoryData.error || productData.error || productAddress) {
        return res.status(400).json({ error: categoryData.error || productData.error || productAddress.error });
    }
    ...
    try {
        ...
        const query = `
            INSERT INTO products (user_id, category_id, subcategory_id, subcategories_items_id, title, currency, price, description,country,city,address)
            VALUES (?, ?, ?, ?, ?, ?, ?, ? , ? ,? ,?)
        `;

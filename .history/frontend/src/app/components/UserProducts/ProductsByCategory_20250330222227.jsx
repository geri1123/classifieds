...
export default function ProductsByCategory({ products, category }) {
  const { openDropdownId, setOpenDropdownId, toggleDropdown, dropdownRef } = useDropdownId();

  // Custom function to handle the dots click
  const handleDotsClick = (e, productId) => {
    e.stopPropagation();
    
    // Explicitly close if already open, otherwise open
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map((product) => (
            <div key={product.id} className="border-2 border-yellow-40 bg-white p-4 rounded-lg flex flex-col relative">
              ...
              {/* 3 Dots Menu Button */}
              <div className='relative w-full flex items-right justify-end'>
                <div className="relative">
                  <HiDotsHorizontal
                    onClick={(e) => handleDotsClick(e, product.id)}

...
router.get("/products", async (req, res) => {
  ...
  try {
    ...
    const processedResults = await Promise.all(results.map(async (product) => {
      ...
      // Get all attribute values for this product using a single query
      const [attributeValues] = await promisePool.query(`
        SELECT 
          a.id, 
          a.name, 
          a.type, 
        ...
        FROM 
          attributes a
        LEFT JOIN 
          product_attributes pa ON a.id = pa.attribute_id AND pa.product_id = ?
        WHERE 
          a.subcategory_id = ?

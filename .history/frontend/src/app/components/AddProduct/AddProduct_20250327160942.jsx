...
export default function AddProduct() {
    ...
    const handlesubmit = async () => {
        try {
            ...
            // Loop through each key of formData and append it to FormData
            Object.keys(formData).forEach(key => {
                if (key === 'product_img') {
                    formData.product_img.forEach(image => {
                        if (!image.isExisting) {
                            // Append new images (excluding existing ones)
                            formDataToSend.append('product_img', image.file); // Assuming image.file is the actual file object
                        }
                    });
                } else {
                    // Append regular fields (non-file fields)
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/add-product`, {
                method: "POST",
                credentials: "include",
                body: formDataToSend,  // Directly send FormData here
            });

...
router.get("/products", async (req, res) => {
  try {
    ...
    const subcategoryIds = products.map(product => product.subcategory_id);

    // Fetch attributes for each subcategory
    const [subcategoryAttributes] = await pool.query(`
      SELECT * FROM attributes WHERE subcategory_id IN (?)
    `, [subcategoryIds]);
    ...
    subcategoryAttributes.forEach(attr => {
      ...
      attributesBySubcategoryId[attr.subcategory_id].push(attr);
    });
    ...
    // Log all fetched attribute values for debugging
    console.log("Fetched attribute values:", JSON.stringify(allAttributesValues, null, 2));
    ...
    allAttributesValues.forEach(attr => {
      ...
      // No type condition, using the value as is
      attributesValuesByProductId[attr.product_id].push({
    ...
    const completeProducts = products.map(product => {
      const productObj = {
        ...
        topAttributes: (attributesValuesByProductId[product.id] || []).slice(0, 3)
      };

      // Create a cleaner attributes object for easier frontend access
      productObj.attributesObject = {};
      (attributesValuesByProductId[product.id] || []).forEach(attr => {
        productObj.attributesObject[attr.attribute_name] = attr.value;
      });
    ...
    console.log("Complete products:", JSON.stringify(completeProducts, null, 2));
...
// Fetch attributes for a specific subcategory

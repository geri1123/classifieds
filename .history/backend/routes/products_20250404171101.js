  ...
  try {
    let query = `
      SELECT 
        ...
        JSON_ARRAYAGG(
          JSON_OBJECT('name', a.name, 'value', pa.value)
        ) AS attributes,
    ...
    const processedResults = results.map((product) => {
      ...
      if (images.length === 0 || (images.length === 1 && images[0] === baseImageUrl)) {
        images = ["https://via.placeholder.com/300x200?text=No+Image"];
      }

      // JSON parse attribute string
      let attributes = [];
      try {
        attributes = JSON.parse(product.attributes);
      } catch (err) {
        console.error("Attribute JSON parse failed:", err);
      }

...
router.post("/add-product", verifyToken, async (req, res) => {
    ...
    const allErrors = {
        ...
        ...userInfo.errors,  // Added user validation errors
    };

    // If any validation errors exist, return them and stop processing
    if (Object.keys(allErrors).length > 0) {
        return res.status(400).json({ errors: allErrors });
    }

    // Now perform the file upload logic after validation passes
    const upload = multer({
        storage,
        fileFilter: (req, file, cb) => {
            const allowedTypes = /jpeg|jpg|png/;

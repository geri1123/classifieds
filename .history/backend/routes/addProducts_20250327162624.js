...
router.post("/add-product", verifyToken, upload.array("product_img", 20), async (req, res) => {
    ...
    db.query(query, [
      ...
      // If there are images, insert them into the product_images table
      ...
      if (req.files && req.files.length > 0) {
        ...
        const imageInsertQuery = 
          `INSERT INTO product_images (product_id, user_id, image_url)
          VALUES ?`;
        const imagesData = productImages.map((image) => [productId, userId, image]);
  
        db.query(imageInsertQuery, [imagesData], (imageErr) => {
          if (imageErr) {

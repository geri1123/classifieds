...
export default function ProductsByCategory({ products, category }) {
  ...
  >
    {product.status_type.toUpperCase()}
              ...
              {/* 3 Dots Menu Button */}
              <div className='relative w-full flex items-right gap-2 justify-end'>
               {product.status_type === "published" ? <button className='border-2 px-2 py-1 border-yellow-40'>
                Deactivate
                </button> : <button className='border border-yellow-40'>
                  Publish
                </button>}

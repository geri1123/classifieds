...
const Step3Form = ({ formData, setFormData, errors, setErrors, onChange }) => {
  const MAX_IMAGES = 20;
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        <label className="block text-gray-700 dark:text-gray-200" htmlFor="video">
          Ngarko video për pronën nga YouTube duke kopjuar linkun URL këtu
        </label>
        <input
          value={formData.video_url}
          onChange={onChange}
          name="video_url"
          className="mt-1 block w-full p-2 text-base border rounded-lg bg-transparent"
          placeholder="p.sh. https://youtube.com/watch?v=[videoId]"
          type="text"
          id="video"
        />
      </div>

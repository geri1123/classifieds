...
router.get('/products', async (req, res) => {
  try {
    ...
    // Query to get all products with the first image only
    const [products] = await connection.query(`
      SELECT p.*, 
             c.name AS category_name, 
             sc.name AS subcategory_name, 
    ...
    // Loop through each product and ensure only the first image is processed
    for (let product of products) {
      if (product.first_image_url) {
        product.product_images = [{
          url: `http://localhost:8081/${product.first_image_url}`,  // Using the first image
          alt: "Product image"  // You can adjust this if there’s an alt field
        }];
      } else {
        product.product_images = [];  // No images if none found
      }

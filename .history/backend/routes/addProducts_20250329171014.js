...
// Function to process category and product data (validation)
const processCategoryData = (body) => {
    const { category_id, subcategory_id, subcategory_item_id } = body;
    const errors = {};
...
const processUserInfo = (body) => {
    ...
    if (!user_firstname) errors.first_name = "First name is required.";
    if (!user_lastname) errors.last_name = "Last name is required.";
    if (!user_email) errors.email = "Email is required.";
    if (!user_phone) errors.phone = "Phone is required.";
    return { user_firstname, user_lastname, user_email, user_phone };
};

// Handle product upload and validation
router.post("/add-product", verifyToken, async (req, res, next) => {
    ...
    // Process category, product data, and user info
    const categoryData = processCategoryData(body);
    const productData = processProductData(body);
    const productAddress = processProductAddress(body);
    const userInfo = processUserInfo(body);
    ...
    // Proceed to handle file upload only if validation is successful
    upload.array("product_img", 20)(req, res, async (err) => {
        if (err) {
            // If multer throws an error (e.g., invalid file type)
        ...
        try {
            ...
            if (files && files.length > 0) {
                const productImages = files.map(file => [productId, userId, `/uploads/${file.filename}`]);
            ...
            if (body.attributes && typeof body.attributes === "object") {
                const attributes = Object.entries(body.attributes).map(([attribute_id, value]) => [
                    productId,
                    attribute_id,
                    value,
                ]);

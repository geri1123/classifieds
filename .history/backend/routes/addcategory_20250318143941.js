...
router.post("/subcategory", async (req, res) => {
    ...
    const checkQuery = "SELECT * FROM subcategories WHERE name = ? AND (category_id = ? OR parent_id = ?)";
    db.query(checkQuery, [name, category_id, parent_id], (err, result) => {
        if (err) {
        ...
        // Insert the new subcategory (under category or another subcategory)
        const insertQuery = "INSERT INTO subcategories (name, category_id, parent_id) VALUES (?, ?, ?)";
...
// Get Subcategories under a specific category or another subcategory
router.get("/subcategories/:parent_id", async (req, res) => {
    const { parent_id } = req.params;
    const query = "SELECT * FROM subcategories WHERE parent_id = ?";
    
    db.query(query, [parent_id], (err, result) => {
        if (err) {
...
router.get("/subcategories", async (req, res) => {
    const query = "SELECT * FROM subcategories"; // Query to fetch all subcategories

...
const processUserInfo = (body) => {
    ...
    if (!user_firstname) errors.user_firstname = "First name is required.";
    if (!user_lastname) errors.user_lastname = "Last name is required.";
    if (!user_email) errors.user_email = "Email is required.";
    if (!user_phone) errors.user_phone = "Phone is required.";
...
router.post("/add-product", verifyToken, async (req, res) => {
    ...
    const connection = await promisePool.getConnection();
    const product_img = req.files.map((file) => `/uploads/${file.filename}`);
    try {
        await connection.beginTransaction(); // Start a transaction
        ...
        // Handle file upload separately after validation and insertion
        upload.array("product_img", 20)(req, res, async (uploadErr) => {
            if (uploadErr) {
                // If upload error occurs, rollback transaction and return error
                await connection.rollback();

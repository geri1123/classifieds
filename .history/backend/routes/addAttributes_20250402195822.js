...
router.post('/AddAttributes', async (req, res) => {
    const { name, subcategory_id , type } = req.body;
    ...
    const insertAttributeQuery = "INSERT INTO attributes (name, subcategory_id , type) VALUES (?, ? , ?)";

    try {
        const [result] = await promisePool.query(insertAttributeQuery, [name, subcategory_id , type]);
        res.status(201).json({ 
...
router.get("/attribute-values/:attribute_id", async (req, res) => {
    const { attribute_id } = req.params;
    const query = "SELECT * FROM attribute_values WHERE attribute_id = ?";

    try {
        const [result] = await promisePool.query(query, [attribute_id]);
        res.json(result);
    } catch (err) {
        console.error("Database error:", err);
        return res.status(500).json({ error: "Error fetching attribute values" });
    }
});

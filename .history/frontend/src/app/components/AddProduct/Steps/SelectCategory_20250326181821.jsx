...
export default function SelectCategory({ formData, changeHandler, setFormData }) {
    const [categories, setCategories] = useState([]);
    const [subcategories, setSubcategories] = useState([]);
    const [childSubcategories, setChildSubcategories] = useState([]); // To store child subcategories
    ...
    useEffect(() => {
        ...
        const fetchSubcategories = async () => {
            try {
                ...
                const data = await response.json();
                setSubcategories(data);
                setChildSubcategories([]); // Reset child subcategories when category changes
    ...
    useEffect(() => {
        ...
        // Filter subcategories that have the selected subcategory as their parent
        const filteredChildSubcategories = subcategories.filter(sub => sub.parent_id === formData.subcategory_id);
        setChildSubcategories(filteredChildSubcategories);
    }, [formData.subcategory_id, subcategories]); // Update when subcategory_id changes

    const handleChildSubcategorySelect = (childId) => {
        // Update formData to reflect the selected child subcategory
        setFormData((prevData) => ({
            ...prevData,
            subcategory_id: childId, // Set the child subcategory as the selected subcategory
        }));
    };
    return (
        <div className="select-category flex flex-col w-screen px-4">
    ...
    <div className="mt-4">
        <h3 className="text-lg font-semibold mb-2">Select child subcategory</h3>
        <div className="flex flex-wrap items-start gap-3">
            {childSubcategories.map((sub) => (
                <button
                    ...
                    name="subcategory_id"
                    value={sub.id}
                    onClick={() => handleChildSubcategorySelect(sub.id)}
                    className={`px-4 py-2 border-2 rounded-lg text-lg cursor-pointer transition-all
                        ${formData.subcategory_id === sub.id
                            ? "bg-yellow-400 text-black border-yellow-500"
                            : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"

      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map((product) => (
            <div key={product.id} className="border-2 border-yellow-40  p-4 rounded-lg flex flex-col relative">
              {/* Product Images */}
              ...
              <div className="relative">
                <HiDotsHorizontal
                  onClick={() => toggleDropdown(product.id)} // Toggle the dropdown visibility for this product
                ...
                {openDropdownId === product.id && (
                  <div
                    ref={dropdownRef}
                    className="absolute bg-gray-100/90 transform transition-all duration-100 ease-in-out animate-fadeInOut bottom-8 right-0 dark:bg-white shadow-light w-32 z-10"
                  >

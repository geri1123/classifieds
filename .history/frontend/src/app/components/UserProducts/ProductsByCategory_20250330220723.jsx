...
export default function ProductsByCategory({ products, category }) {
  const { openDropdownId, setOpenDropdownId, dropdownRef } = useDropdownId();
  
  const handleToggleDropdown = (productId) => {
    if (openDropdownId === productId) {
      openDropdownId(null); // Close the dropdown if it's already open
    } else {
      openDropdownId(productId); // Open the dropdown if it's not open
    }
  };

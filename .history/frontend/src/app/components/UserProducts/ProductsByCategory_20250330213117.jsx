...
export default function ProductsByCategory({ products, category }) {
  ...
  const handleDropdownToggle = (productId) => {
    // If clicking on the same product that has an open dropdown, close it
    if (openDropdownId === productId) {
      toggleDropdown(null); // Pass null to close the dropdown
    } else {
      toggleDropdown(productId); // Otherwise, open the dropdown for this product
    }
  };

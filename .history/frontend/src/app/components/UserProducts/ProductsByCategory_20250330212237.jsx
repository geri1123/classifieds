...
export default function ProductsByCategory({ products, category }) {
  ...
  const handleDropdownToggle = (productId) => {
    // If the dropdown is already open for the current product, close it
            ...
            <div key={product.id} className="border-2 border-yellow-40 bg-white p-4 rounded-lg flex flex-col relative">
              ...
              <div className="relative">
                <HiDotsHorizontal
                  ...
                  onClick={() => handleDropdownToggle(product.id)} // Toggle the dropdown visibility for this product
                  className="hover:bg-blue-yellow-400 hover:text-white duration-300 ease-in-out transition-all text-[20px] border-blue-500 text-blue-500 dark:text-blue-200 dark:hover:bg-blue-200 dark:hover:text-black cursor-pointer border-1 dark:border-blue-200 rounded w-8 h-7 flex items-center justify-center"
                />
                ...
                {openDropdownId === product.id && (
                  <div
                    ref={dropdownRef}
                    className="absolute bg-white shadow-lg border-1 border-yellow-40 transform transition-all duration-100 ease-in-out animate-fadeInOut bottom-8 right-0 dark:bg-white shadow-light w-32 z-10"
                  >

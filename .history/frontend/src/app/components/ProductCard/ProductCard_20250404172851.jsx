    ...
    <Link href={`/product/${id}`}>
      <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:shadow-lg hover:scale-105 h-full flex flex-col">
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          {/* Product Attributes - show key attributes */}
          {attributesObject && (
            <div className="mt-auto pt-3 border-t border-gray-200">
              <div className="flex flex-wrap gap-2 text-xs">
                {/* Show car specific attributes */}
                {attributesObject['Condition'] && (
                  <span className="bg-gray-100 px-2 py-1 rounded-full">
                    {attributesObject['Condition']}
                  </span>
                )}
                {attributesObject['Build year'] && (
                  <span className="bg-gray-100 px-2 py-1 rounded-full">
                    {attributesObject['Build year']}
                  </span>
                )}
                {attributesObject['Fuel'] && (
                  <span className="bg-gray-100 px-2 py-1 rounded-full">
                    {attributesObject['Fuel']}
                  </span>
                )}
                {/* Show house specific attributes */}
                {attributesObject['Bedrooms'] && (
                  <span className="bg-gray-100 px-2 py-1 rounded-full">
                    {attributesObject['Bedrooms']} bd
                  </span>
                )}
                {attributesObject['Bathrooms'] && (
                  <span className="bg-gray-100 px-2 py-1 rounded-full">
                    {attributesObject['Bathrooms']} ba
                  </span>
                )}
                {attributesObject['Square feet'] && (
                  <span className="bg-gray-100 px-2 py-1 rounded-full">
                    {attributesObject['Square feet']} sqft
                  </span>
                )}
              </div>
            </div>

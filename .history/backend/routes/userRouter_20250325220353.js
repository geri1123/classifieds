...
router.put("/add-social-media", verifyToken, (req, res) => {
  const userId = req.userId;
  const { instagram, facebook, linkedin } = req.body;

  const updateQuery = `
      INSERT INTO social_media (user_id, instagram, facebook, linkedin)
      VALUES (?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE instagram = VALUES(instagram), facebook = VALUES(facebook), linkedin = VALUES(linkedin);
  `;

  db.query(updateQuery, [userId, instagram, facebook, linkedin], (err, result) => {
      if (err) {

...
router.get("/products", async (req, res) => {
  ...
  try {
    // Base query to get products and related info with improved attribute formatting
    let query = `
      SELECT 
        p.*, 
        ...
        -- Explicitly handle null values and format consistently
        GROUP_CONCAT(
          DISTINCT 
          CASE 
            WHEN a.name IS NOT NULL AND pa.value IS NOT NULL 
            THEN CONCAT(a.name, ':', pa.value) 
            ELSE NULL 
          END
          SEPARATOR ', '
        ) AS attributes,
    ...
    if (results.length > 0) {
      console.log("Sample product attributes:", results[0].attributes);
    }
    ...
    const processedResults = results.map((product) => {
      ...
      let images = (product.images?.split(',') || []).map((url) => `${baseImageUrl}${url}`);

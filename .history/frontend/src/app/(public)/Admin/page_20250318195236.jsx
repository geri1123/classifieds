...
//     // Filter subcategories based on selected category and parent subcategory
...
//                             {subcategories
//                                 .filter((childSub) => childSub.parent_id === sub.id)
//                                 .map((childSub) => (
//                                     <li key={childSub.id}>📁 {childSub.name}</li>
//                                 ))}
//                         </ul>
//                     </li>
//                 ))}
...
export default function ManageAttributes() {
    ...
    const [selectedSubcategoryForAttribute, setSelectedSubcategoryForAttribute] = useState("");
    ...
    async function addAttribute() {
        if (!newAttribute.trim() || !selectedSubcategoryForAttribute) {
            setAttributeErrorMessage("Attribute name and subcategory are required!");
            return;
        }
        ...
        <div className="p-6 max-w-5xl mx-auto">
            ...
            <div className="bg-gray-100 p-4 rounded-lg shadow mt-6">
                ...
                <select
                    ...
                    value={selectedSubcategoryForAttribute}
                    onChange={(e) => setSelectedSubcategoryForAttribute(e.target.value)}
                >
                    <option value="">Select Subcategory</option>
                    {subcategories
                        .filter(sub => !sub.parent_id) // Only top-level subcategories
                        .map((subcategory) => (
                ...
                />
                {attributeErrorMessage && <p className="text-red-500 text-sm mt-1">{attributeErrorMessage}</p>}
                <button
            ...
            {/* Existing Attributes */}
            <div className="mt-6">
                <h2 className="text-lg font-semibold mb-2">Existing Attributes</h2>
...
//     const [categoryErrorMessage, setCategoryErrorMessage] = useState("");
//     const [subcategoryErrorMessage, setSubcategoryErrorMessage] = useState("");
//     const [atributeErrorMessage , setAttributeErrorMessage]=('');
//     const [attributes, setAttributes] = useState([]);
//     const [newAttribute, setNewAttribute] = useState("");
//     // Fetch categories
//     async function fetchCategories() {
...
//                 parent_id: selectedParentSubcategory 
//             };
...
//                 {categories.map((category) => (
//                     <div key={category.id} className="bg-gray-100 p-4 max-h-[200px] overflow-y-auto rounded-lg shadow">

      ...
      <div className="flex  flex-col items-start ml-6 gap-2  ">
        ...
        <button onClick={() => setIsOpen(true)} className="flex gap-3 items-center">
          ...
          {user?.phone  && <FaPhoneAlt size={17} />}
            <span className={`border-b border-b-gray-900 dark:border-b-gray-100 ${user?.phone ? 'text-gray-800 dark:text-gray-200' : 'text-red-700 italic flex items-center gap-1'}`}>
              {!user?.phone && <FaExclamationCircle className="text-red-700" />}
              {user?.phone || ' Please Enter a phone number !'}
            </span>
          </h2>

...
router.post("/forgot-password", async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "We have emailed the link to reset your password!", expiresAt: expirationTime });
  } catch (error) {
...
router.post("/reset-password", async (req, res) => {
  ...
  try {
    ...
    // Update Password
    await promisePool.query("UPDATE users SET user_password = ? WHERE user_id = ?", [hashedPassword, userId]);

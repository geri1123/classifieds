...
router.post("/social-media" , verifyToken , (res, req)=>{
  const userId=req.userId;
  const {social_media , link} = req.body;
  if(!social_media || !link){
    return res.status(400).json({error:"Please fill all fields"});
  }
  const socialMediaQuery="INSERT INTO `social_media` (user_id , social_media , link) VALUES (?,?,?)";
  db.query(socialMediaQuery , [userId , social_media , link] , (err, result)=>{
    if(err){
      console.error("Error inserting social media:", err);
      return res.status(500).json({ error: "Server error" });
    }
    res.json({success:true , message:"Social media added successfully"});
  })
})

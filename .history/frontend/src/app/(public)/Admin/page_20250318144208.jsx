...
export default function ManageAttributes() {
    ...
    async function addCategory() {
        ...
        try {
            const response = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/category`, { name: newCategory });
    ...
    async function addSubcategory() {
        if (!newSubcategory.trim() || (!selectedCategory && !selectedParentSubcategory)) {
            setSubcategoryErrorMessage("Category or parent subcategory is required!");
            return;
        }
        try {
            ...
            setSubcategories([...subcategories, { id: response.data.subcategoryId, name: newSubcategory, parent_id: selectedParentSubcategory }]);
            setNewSubcategory("");
        ...
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                <div className="bg-gray-100 p-4 rounded-lg shadow">
                    ...
                    <input type="text" className="border p-2 w-full rounded" placeholder="Category Name" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} />
                    {categoryErrorMessage && <p className="text-red-500 text-sm mt-1">{categoryErrorMessage}</p>}
                    <button onClick={addCategory} className="bg-blue-500 text-white px-4 py-2 mt-2 w-full rounded hover:bg-blue-600">Add Category</button>
                </div>

                {/* Add Subcategory Form */}
                <div className="bg-gray-100 p-4 rounded-lg shadow">
                    <h2 className="text-lg font-semibold mb-2">Add Subcategory</h2>
                    <select className="border p-2 w-full rounded" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)}>
                        <option value="">Select Category</option>
                        {categories.map((category) => (<option key={category.id} value={category.id}>{category.name}</option>))}
                    </select>
                    <select className="border p-2 w-full rounded mt-2" value={selectedParentSubcategory} onChange={(e) => setSelectedParentSubcategory(e.target.value)}>
                        <option value="">Select Parent Subcategory (Optional)</option>
                        {subcategories.map((sub) => (<option key={sub.id} value={sub.id}>{sub.name}</option>))}
                    </select>
                    <input type="text" className="border p-2 w-full rounded mt-2" placeholder="Subcategory Name" value={newSubcategory} onChange={(e) => setNewSubcategory(e.target.value)} />
                    {subcategoryErrorMessage && <p className="text-red-500 text-sm mt-1">{subcategoryErrorMessage}</p>}
                    <button onClick={addSubcategory} className="bg-green-500 text-white px-4 py-2 mt-2 w-full rounded hover:bg-green-600">Add Subcategory</button>
                </div>
            </div>
                    ...
                    categories.map((category) => (
                        <div key={category.id} className="mb-4">
                            ...
                            <ul className="ml-4 list-disc">
                                {subcategories.filter((sub) => sub.category_id === category.id || sub.parent_id).map((sub) => (
                                    <li key={sub.id} className="ml-4">📁 {sub.name}</li>
                                ))}
                            </ul>
                        </div>
                    ))

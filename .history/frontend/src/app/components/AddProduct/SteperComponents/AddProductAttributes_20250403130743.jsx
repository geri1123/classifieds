...
export default function AddProductAttributes({ formData, setFormData }) {
    ...
    // Function to format checkbox values for display
    const formatCheckboxValues = (values) => {
        if (!values || !Array.isArray(values) || values.length === 0) return "None";
        
        // Return values as a comma-separated string with line breaks
        ...
        if (attribute.type.toLowerCase() === "option") {
            ...
            return (
                <div key={attribute.id} className="mb-6">
                    ...
                    {/* This hidden div shows how the data is being stored - for debugging */}
                    <div className="mt-2 text-xs text-gray-500">
                        <p>Selected values:</p>
                        <pre className="bg-gray-100 p-1 mt-1">
                            {formData.attributes?.[attribute.id] ? 
                                formatCheckboxValues(formData.attributes[attribute.id]) : 
                                "None"}
                        </pre>
                    </div>
                </div>

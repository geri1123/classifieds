...
const imageFilter = (req, file, cb) => {
  ...
  if (extname && mimetype) {
    ...
    cb(new Error("Only images (JPEG, PNG) are allowed for product images"));
  }
};

// File filter for PDF documents only
...
// Setup separate uploads for images and documents
...
router.post(
  ...
  (req, res, next) => {
    const upload = multer({
      ...
      fileFilter: (req, file, cb) => {
        ...
        } else if (file.fieldname === 'product_doc') {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (req.files && req.files.product_doc) {
        for (const file of req.files.product_doc) {
          const filename = `${Date.now()}-${file.originalname.replace(/\s+/g, "_")}`;

...
//             return (
//                 <div key={`attribute-${attribute.id}`} className="mb-6 flex gap-6">
//                     <p className="text-sm font-medium text-gray-700">{attribute.name}</p>
//                     <div className="grid grid-cols-2 gap-2">
//                         {checkboxValues.map((value) => (
//                             <label key={`attribute-${attribute.id}-value-${value.id}`} className="flex items-center space-x-2">
//                                 <input
//                                     type="checkbox"
//                                     id={`attribute-${attribute.id}-${value.id}`}
//                                     name={`attribute-${attribute.id}`}
...
export default function AddProductAttributes({ formData, setFormData }) {
    ...
    const handleCheckboxChange = (e, attributeId, attributeValueId, attributeValueName) => {
        ...
        setFormData((prevData) => {
            // Get existing selected values (default to empty object)
            const prevSelected = prevData.attributes?.[attributeId] || {};
            
            // Update the checkbox value
            let updatedValues = {
                ...prevSelected,
                [attributeValueId]: checked ? attributeValueName : null
            };
            
            // Remove any null values (unchecked items)
            ...
            return (
                <div key={`attribute-${attribute.id}`} className="mb-6 flex gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {checkboxValues.map((value) => (
                            <label 
                                key={`attribute-${attribute.id}-value-${value.id}`} 
                            ...
                            >
                                <input
                                    ...
                                    id={`attribute-${attribute.id}-${value.id}`}
                                    name={`attribute-${attribute.id}`}
                                    checked={Boolean(formData.attributes?.[attribute.id]?.[value.id])}

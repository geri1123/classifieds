...
// Updated helper function to categorize files
...
router.post("/add-product", verifyToken, upload.array("files", 20), async (req, res) => {
  const userId = req.userId;
  const io = req.app.get("socketio"); // Get Socket.io instance
  ...
  try {
    ...
    const { images, documents } = categorizeFiles(req.files || []);

    // Save images and documents to disk and record in database
    const savedFiles = {
      images: [],
      documents: []
    };
    ...
    for (const file of images) {
      ...
      await connection.query(
        "INSERT INTO product_images (product_id, user_id, image_url) VALUES (?, ?, ?)", 
        [productId, userId, fileUrl]
      );
    }
    ...
    for (const file of documents) {
      ...
      // Insert document into product_documents table
      await connection.query(
        "INSERT INTO product_documents (product_id, user_id, document_url) VALUES (?, ?, ?)", 
        [productId, userId, fileUrl]
      );
    }
    ...
    res.status(201).json({
      ...
      message: notificationMessage,
      files: {
        images: savedFiles.images.map((file) => ({ url: file.url, name: file.originalname })),
        documents: savedFiles.documents.map((file) => ({ url: file.url, name: file.originalname }))
      }
    });
  } catch (err) {
    await connection.rollback();

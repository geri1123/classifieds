...
const processProductData = (body) => {
    const { title , currency , price } = body;
...
router.post("/add-product", verifyToken, async (req, res) => {
    ...
    db.query(query, [userId, categoryData.category_id, categoryData.subcategory_id, categoryData.subcategoryItemId, productData.title , productData.currency , productData.price], (err, result) => {
        if (err) {

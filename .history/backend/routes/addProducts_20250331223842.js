...
const { sendPropertyStatusEmail } = require("../services/emailService");
...
router.put("/update-product-status/:id", verifyToken, async (req, res) => {
  ...
  if (!["published", "draft", "onhold"].includes(status_type)) {
    return res.status(400).json({ error: "Invalid status type" });
  }

  try {
    // Fetch property details before updating
    const [propertyData] = await promisePool.query(
      "SELECT title, user_id, email FROM products JOIN users ON products.user_id = users.id WHERE products.id = ?",
      [productId]
    );

    if (propertyData.length === 0 || propertyData[0].user_id !== userId) {
      return res.status(403).json({ error: "Unauthorized or property not found" });
    }

    const propertyTitle = propertyData[0].title;
    const userEmail = propertyData[0].email;
    ...
    await sendPropertyStatusEmail(userEmail, status_type, productId, propertyTitle);
...
//     // 1. Fetch product images to delete from the server
//     const [images] = await connection.query(
//       "SELECT image_url FROM product_images WHERE product_id = ?",
//       [productId]
//     );

//     // 2. Delete product images from the filesystem
//     for (const image of images) {
//       const filePath = path.join(__dirname, "../", image.image_url); // Resolve full path
//       if (fs.existsSync(filePath)) {
//         fs.unlinkSync(filePath); // Delete the file
//       }
//     }

//     // 3. Delete product images from the database

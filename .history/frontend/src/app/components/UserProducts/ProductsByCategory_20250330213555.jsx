...
export default function ProductsByCategory({ products, category }) {
  ...
  const handleDropdownToggle = (productId, e) => {
    e.stopPropagation(); // Prevent event bubbling
    
    // Only open the dropdown if it's currently closed for this product
    if (openDropdownId === productId) {
      // If already open, set to null to close it
      toggleDropdown(null);
    } else {
      // If closed, open it
          ...
          {products.map((product) => (
            <div key={product.id} className="border-2 border-yellow-40 bg-white p-4 rounded-lg flex flex-col relative">
              ...
              {product.images && product.images.length > 0 ? (
                ...
                <img src="" alt={product.title} className="w-full h-48 object-cover rounded-lg" />
              )}

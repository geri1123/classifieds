...
export default function SelectCategory({ formData, changeHandler, setFormData }) {
    ...
    return (
        <div className="select-category flex flex-col min-h-screen items-start justify-start px-4 py-4">
            <h2 className="text-xl font-semibold mb-4">Please select a listing type and category for your listing.</h2>
            <div className="w-full">
                {loading ? (
                    <p>Loading categories...</p>
                ) : (
                    categories.length > 0 && categories.map((category) => (
                        <label key={category.id} className="flex items-center space-x-2 cursor-pointer mb-2">
                            <input
                                type="radio"
                                name="category_id"
                                value={category.id}

...
router.put("/update-product-status/:id", verifyToken, async (req, res) => {
  ...
  const userId = req.user.id; // Assuming `verifyToken` middleware adds user data to `req.user`
  ...
  try {
    // Fetch the product from the database to check its owner
    const [product] = await promisePool.query("SELECT * FROM products WHERE id = ?", [productId]);
    ...
    const updateQuery = `UPDATE products SET status_type = ? WHERE id = ?`;
    const [result] = await promisePool.query(updateQuery, [status_type, productId]);

    if (result.affectedRows === 0) {
      return res.status(500).json({ error: "Failed to update product status" });
    }

    res.status(200).json({ success: true, message: "Product status updated successfully", newStatus: status_type });
  } catch (error) {

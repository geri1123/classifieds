  ...
  return (
    <>
      <nav className="bg-white shadow-md">
        <div className="max-w-full px-4 sm:px-10 lg:px-20 mx-auto">
          {/* Top Bar */}
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-6">
              ...
              {isAuthenticated  ? (
               <DropdownMenu align="right" user={user} logoutUser={logoutUser}/>
            ) : (
              <FiUser onClick={toggleLoginDropdown} className="w-6 h-6 text-black" /> 
            )}
            </div>
          ...
          <div className="flex items-center border-t py-3">
            ...
            <div className="flex space-x-6 ml-4 text-gray-700 font-medium">
              ...
              {/* <a href="/Products" className="py-1 rounded-md">Property</a>
              ...
              {categories.map((cat)=>(
               return <a key={cat.id} href="#" className="py-1 rounded-md">{cat.name}</a>
              ))}
            </div>

            {/* Post an Ad Button */}
            <button onClick={handleclick} className="ml-auto border-2 border-yellow-40 text-black px-4 py-2 rounded-md font-medium">
              Post an Ad
            </button>
          </div>
        </div>
        ...
        >
          <div
            ...
            className={`before:absolute rounded-xl before:inset-0 before:backdrop-blur-md 
              before:bg-white dark:before:bg-gray-800/90 before:-z-10 z-30 mt-20 mb-20 shadow-lg ${isLoginClosing ? 'animate-closeModal' : 'animate-openModal'}`}
          >

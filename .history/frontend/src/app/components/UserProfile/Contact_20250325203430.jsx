...
export function Contact({ user, mutate }) {
  ...
  const isDisabled = loading || !userData.phone || userData.phone === user?.phone;

  return (
    <div>
      <h2 className="flex mb-4 items-center text-gray-800 gap-2 text-lg font-semibold dark:text-gray-200 text-left w-full">
        <MdOutlineContactSupport /> Kontaktet
      </h2>
      ...
      >
        <h2 className="dark:text-sky-400 text-sky-800 mb-4 text-xl">
          {user?.phone ? 'Ndrysho Numrin e telefonit' : 'Shto Numrin e telefonit'}
        </h2>
...
/>
          ...
          <Button type="submit" disabled={isDisabled}>
            {loading ? 'Loading...' : user?.phone ? 'Ndrysho' : 'Shto'}
          </Button>
        </form>
      </ModalDropdown>
    </div>

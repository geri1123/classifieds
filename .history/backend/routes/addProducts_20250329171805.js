...
// Configure multer storage but don't apply it yet
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, "uploads/");
  },
...
// Create the multer instance but don't use it as middleware yet
const upload = multer({
  storage,
  fileFilter: (req, file, cb) => {
    const allowedTypes = /jpeg|jpg|png/;
...
// The route handler first applies verifyToken, then validates, and only then handles file upload
router.post("/add-product", verifyToken, (req, res, next) => {
  // First validate all required fields before handling the file upload
  const categoryData = processCategoryData(req.body);
  const productData = processProductData(req.body);
  const productAddress = processProductAddress(req.body);
  const userInfo = processUserInfo(req.body);
...
// Apply multer middleware after validation
upload.array("product_img", 20), 
// Final handler after validation and file upload
async (req, res) => {
  const userId = req.userId;
  console.log("Uploaded Files:", req.files);

  // Process validated data again
  const categoryData = processCategoryData(req.body);
  const productData = processProductData(req.body);
  const productAddress = processProductAddress(req.body);
  const userInfo = processUserInfo(req.body);
  ...
  try {
    ...
    // If there are uploaded files, delete them on error
    if (req.files && req.files.length > 0) {

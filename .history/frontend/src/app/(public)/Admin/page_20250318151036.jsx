...
export default function ManageAttributes() {
    ...
    // UseEffect to fetch categories and subcategories on mount
    useEffect(() => {
        fetchCategories();
        fetchSubcategories();
    }, []);
    ...
    const filteredSubcategories = subcategories.filter(sub => 
        sub.category_id === selectedCategory || sub.parent_id === selectedParentSubcategory
    );
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                <div className="bg-gray-100 p-4 rounded-lg shadow">
                    ...
                    >
                        ...
                        {subcategories
                            .filter(sub => sub.category_id === selectedCategory) // Filter parent subcategories
                            .map((sub) => (
                    ...
                    categories.map((category) => (
                        <div key={category.id} className="mb-4">
                            ...
                            <ul className="ml-4 list-disc">
                                ...
                                {/* Nested subcategories under a parent */}
                                {filteredSubcategories
                                    .filter(sub => sub.category_id === category.id && sub.parent_id)
                                    .map((sub) => (
                                        <li key={sub.id} className="ml-4">📂 {sub.name} (Nested)</li>
                                    ))}
                            </ul>
                        </div>
                    ))

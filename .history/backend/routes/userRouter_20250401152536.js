...
router.post("/update-email", verifyToken, async (req, res) => {
  ...
  try {
      ...
      await db.query("INSERT INTO notifications (user_id, message, status) VALUES (?, ?, 'unread')", 
                    [userId, notificationMessage]);

      // Emit a socket event after 10 seconds
      setTimeout(async () => {
          ...
          if (notification.length > 0) {
              io.to(`user_${userId}`).emit("new_notification", notification[0]);
          }
...
router.post("/update-username", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Use async function inside setTimeout
    setTimeout(async () => {
      let notificationMessage = "Emri juaj i përdoruesit është ndryshuar. Ju nuk mund ta ndryshoni emrin e përdoruesit për 10 ditët e ardhshme.";
    ...
    }, 10000);  // Timeout duration in ms (10 seconds)
    ...
    const checkUsernameQuery = `
      ...
      WHERE username = ?
        AND user_id != ?
    `;
    const [usernameResult] = await db.query(checkUsernameQuery, [username, userId]);
...
router.post("/change-password", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const [results] = await promisePool.query("SELECT email, password, receive_notifications FROM `users` WHERE user_id = ?", [userId]);
    ...
    // Update the password in the database using promisePool
    await promisePool.query("UPDATE `users` SET password = ? WHERE user_id = ?", [hashedNewPassword, userId]);

    // Optionally send a notification if the user wants

...
export function SocialMedia() {
      ...
      try {
        const response = await fetch("/social-media", {
          ...
          headers: {
            ...
            // Include token if required for authorization
  ...
  return (
    <div className="social-media">
      ...
      {/* No data available */}
      {socialMediaData && socialMediaData.length === 0 && <p>No social media available</p>}

      {/* Display social media data */}
      {socialMediaData && socialMediaData.length > 0 && (
        <ul>
          {socialMediaData.map((social, index) => (
            <li key={index}>{social.platform}: {social.link}</li>
          ))}
        </ul>
      )}
    </div>

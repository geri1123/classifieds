...
// Middleware to parse non-file data
router.use(express.json());
router.use(express.urlencoded({ extended: true }));

// Function to process product data (validation)
const processCategoryData = (body) => {
    const { category_id, subcategory_id, subcategory_item_id } = body;
    const errors = {};
...
router.post("/add-product", verifyToken, async (req, res) => {
    const userId = req.userId;
    console.log("Request Body:", req.body); // Log the request body to check if fields are properly passed
    ...
    // Debugging log to check if validation errors exist
    ...
    // Proceed to file upload only after validation passes
    upload.array("product_img", 20)(req, res, async (err) => {
        ...
        // Debugging log to verify that files are uploaded
        console.log("Uploaded Files:", req.files);

        // Proceed with database interaction after file upload
        const connection = await promisePool.getConnection();
        try {
            await connection.beginTransaction(); // Start a transaction

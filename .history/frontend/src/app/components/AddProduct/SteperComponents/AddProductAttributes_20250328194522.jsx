...
export default function AddProductAttributes({ formData, setFormData }) {
    ...
    const renderInputField = (attribute) => {
        ...
        if (
            ["build year", "rooms", "bathrooms" , "Number of Floors" , "Built Year"].includes(attribute.name.toLowerCase())
        ) {
            // If attribute is build year, rooms, or bathrooms, show number input
            return (
                <InputField
                    label={attribute.name}
                    value={formData.attributes[attribute.id] || ""}

...
async function fetchProducts(category, status) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/user-products?category=${category}&status=${status}`, {
      method: 'GET',
      // cache: 'no-store',
...
export default async function ProductsPage({ searchParams }) {
  // Convert searchParams to a Promise and await its resolution
  const resolvedSearchParams = await searchParams;
  const category = resolvedSearchParams.category || 'All';
  const status = resolvedSearchParams.status || '';
  const data = await fetchProducts(category, status);

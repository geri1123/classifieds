...
export function SocialMedia({ user, mutate }) {
  ...
  const fetchSocialMedia = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/social-media`,
        {
          method: "GET",
          credentials: "include",
      ...
      if (Array.isArray(data) && data.length > 0) {
        setSocialMediaData(data[0]); // Store first record if array
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/add-social-media`,
        {
          method: "PUT",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
      ...
      if (result.error) {
        ...
        fetchSocialMedia(); // Refetch updated data

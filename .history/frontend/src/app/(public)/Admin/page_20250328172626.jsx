...
const [newSubcategoryItem, setNewSubcategoryItem] = useState("");
    ...
    async function fetchAttributes(subcategory_id) {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/attributes/${subcategory_id}`);
            setAttributes((prev) => ({
                ...prev,
                [subcategory_id]: response.data, // Store attributes by subcategory_id
            }));
        } catch (error) {
    ...
    async function addAttribute() {
        try {
            ...
            setAttributes((prev) => ({
                ...prev,
                [selectedParentSubcaategoryAtt]: [
                    ...(prev[selectedParentSubcaategoryAtt] || []),
                    { id: response.data.attributeId, name: newAttribute },
                ],
            }));
        } catch (error) {

...
router.post('update-social-media' , verifyToken , async(req,res)=>{
  const userId=req.userId;
  const {instagram , facebook , linkedin , fiverr}=req.body;
  const db="UPDATE `users` SET instagram = ? , facebook = ? , linkedin = ? , fiverr=?  WHERE user_id = ?";
  db.query(db, [instagram , facebook , linkedin , fiverr , userId] , (err, result)=>{
    if(err){
      console.error("Error updating user:", err);
      return res.status(500).json({ error: "Server error" });
    }

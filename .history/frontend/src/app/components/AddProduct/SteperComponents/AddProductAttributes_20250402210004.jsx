...
export default function AddProductAttributes({ formData, setFormData }) {
    ...
    // Fetch attributes and attribute values whenever the subcategory changes
    useEffect(() => {
        if (formData.subcategory_id) {
            dispatch(fetchAttributes(formData.subcategory_id));
        }
    ...
    useEffect(() => {
        attributes.forEach((attribute) => {
            if (attribute.type === "option") {
                dispatch(fetchAttributeValues(attribute.id)); // Fetch attribute values for "option" type attributes
            }
        });
    }, [attributes, dispatch]);
    ...
    const renderInputField = (attribute) => {
        if (attribute.type.toLowerCase() === "option") {
            // Ensure attributeValues[attribute.id] is an array before rendering SelectInput
            const options = Array.isArray(attributeValues[attribute.id]) ? attributeValues[attribute.id] : [];

            // If attribute type is "option", show a dropdown
            return (
                <SelectInput
                    key={attribute.id}
                    label={attribute.name}

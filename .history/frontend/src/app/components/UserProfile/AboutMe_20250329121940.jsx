    ...
    return (
        <div className="bg-white w-full p-6 gap-3 h-full dark:bg-gray-800 rounded-lg shadow-md flex flex-col items-center">
           <div className="flex items-center justify-between w-full"> 
            ...
            <div className="border scrollbar xl:h-40 overflow-y-auto rounded-lg p-3 dark:border-gray-200 border-yellow-40 border-2 w-full break-words whitespace-normal">
            ...
            <h2 className="dark:text-sky-400 text-sky-800  mb-4 text-xl">Change About Me</h2>
            {errors && (
        <p className="text-sm text-red-600 mb-1">{errors}</p>
      )}
       {successMessage && <p className="text-green-500">{successMessage}</p>}
               <form onSubmit={handlesubmit}>
              <textarea  value={aboutMe} name='about_me'  onChange={(e) => setAboutMe(e.target.value)} className='bg-transparent mb-4 w-full'/>

...
export default function AddProductAttributes({ formData, setFormData }) {
    ...
    // Fetch attribute values for all option-type attributes
    useEffect(() => {
        if (attributes && attributes.length > 0) {
    ...
    const handleAttributeChange = (e, attributeId, attributeType) => {
        ...
        // For option type attributes, we need to store both value_id and the display value
        if (attributeType === 'option') {
            // Find the selected option's display value
            const selectedOption = attributeValues.find(val => val.id.toString() === value.toString());
            ...
            // For other types, just store the direct value
            setFormData(prevData => ({
                ...prevData,
                attributes: {
                    ...prevData.attributes,
    ...
    if (!attributes || attributes.length === 0) {
        return (
            <div className="bg-gray-100 w-full flex flex-col gap-3 p-4 rounded-xl">
                <h2 className="text-lg font-semibold">No product specifications for this subcategory</h2>
            </div>
        );
    }
    ...
    return (
        <div className="bg-gray-100 w-full flex flex-col gap-3 p-4 rounded-xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {attributes.map((attribute) => {
                    // Get values for this attribute if it's option type
                    const options = attributeValues.filter(val => val.attribute_id === attribute.id)
                        .map(val => ({ id: val.id, name: val.id, symbol: val.value }));
                        ...
                        <div key={attribute.id} className="mb-3">
                            {attribute.type === "option" ? (
                                ...
                                <div className="flex items-center">
                                    <label className="mr-2">{attribute.name}</label>
                                    <InputField
                                        type="checkbox"
                                        checked={formData.attributes?.[attribute.id]?.value === "true"}

...
router.get("/social-media", verifyToken, (req, res) => {
  const userId = req.userId;
  const socialMediaQuery = "SELECT instagram, facebook, linkedin, fiverr FROM `social_media` WHERE user_id = ?";
  
  db.query(socialMediaQuery, [userId], (err, result) => {
      if (err) {
...
router.put("/add-social-media", verifyToken, (req, res) => {
  const userId = req.userId;
  const { instagram, facebook, linkedin, fiverr } = req.body;
  ...
  const deleteQuery = "DELETE FROM social_media WHERE user_id = ?"; // Step 1: Delete old data
  const insertQuery = `
      INSERT INTO social_media (user_id, instagram, facebook, linkedin, fiverr)
      VALUES (?, ?, ?, ?, ?) 
  `; // Step 2: Insert new data

  db.query(deleteQuery, [userId], (deleteErr) => {
    if (deleteErr) {
      console.error("Error deleting social media:", deleteErr);
      return res.status(500).json({ error: "Server error while deleting" });
    }

    db.query(insertQuery, [userId, instagram, facebook, linkedin, fiverr], (insertErr, result) => {
      if (insertErr) {
        console.error("Error inserting social media:", insertErr);
        return res.status(500).json({ error: "Server error while inserting" });
      }
      res.json({ message: "Social media replaced successfully" });
    });
  });
});

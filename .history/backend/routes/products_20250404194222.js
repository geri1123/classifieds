...
router.get("/products", async (req, res) => {
  try {
    ...
    // Get all subcategory IDs from products
    const subcategoryIds = [...new Set(products.map(product => product.subcategory_id).filter(id => id != null))];
    
    // Get ALL attributes for each subcategory
    const [allSubcategoryAttributes] = await pool.query(`
      SELECT 
        a.id,
        a.name,
        a.subcategory_id,
    ...
    // Get existing attribute values for each product
    const [allAttributeValues] = await pool.query(`
      SELECT 
        pa.product_id,
        pa.attribute_id,
        pa.value
    ...
    // Create a map of attributes by subcategory_id
    const attributesBySubcategoryId = {};
    allSubcategoryAttributes.forEach(attr => {
      if (!attributesBySubcategoryId[attr.subcategory_id]) {
        attributesBySubcategoryId[attr.subcategory_id] = [];
      }
      attributesBySubcategoryId[attr.subcategory_id].push({
        id: attr.id,
        name: attr.name,
        type: attr.type
      });
    ...
    // Create a map of attribute values by product_id and attribute_id
    const valuesByProductAndAttribute = {};
    allAttributeValues.forEach(val => {
      const key = `${val.product_id}_${val.attribute_id}`;
      valuesByProductAndAttribute[key] = val.value;
    });
    ...
    const completeProducts = products.map(product => {
      ...
      const subcategoryAttributes = product.subcategory_id ? 
        (attributesBySubcategoryId[product.subcategory_id] || []) : [];
      
      // Map each attribute to include its value for this product (if exists)
      const productAttributes = subcategoryAttributes.map(attr => {
        const key = `${product.id}_${attr.id}`;
        return {
          id: attr.id,
          name: attr.name,
          type: attr.type,
          value: valuesByProductAndAttribute[key] || null
        };
      });
